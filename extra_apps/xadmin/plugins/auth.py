...
class UserAdmin(object):
    ...
    list_display = ('username', 'email',
                    'first_name', 'last_name', 'is_staff')                  # 显示字段（函数也可以）
    list_filter = ('is_staff', 'is_superuser', 'is_active')                         # 过滤器
    search_fields = ('username', 'first_name', 'last_name', 'email')                # 搜索栏
    ordering = ('username',)                                                        # 排序
    style_fields = {'user_permissions': 'm2m_transfer'}                             # 字段使用的样式
    model_icon = 'fa fa-user'                                                       # 栏目图标
    # 以AJAX方式加载外键字段：在课程管理中在输入关键字段时才显示列表
